...
export default function Table_basketball() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://www.liveresult.ru/basketball/USA/NBA/standings?st=0');
          ...
          <table className='table__common'>
            ...
            <tbody>
              {tableData.map((row, index) => (
                <tr key={index}>
                  <td>{row.position}</td>
                  <td><div className="img16">
                  <img src={row.logo} alt="Логотип команды" />
                  <td>{row.team}</td>
                  </div></td> {/* Отображаем логотип */}
